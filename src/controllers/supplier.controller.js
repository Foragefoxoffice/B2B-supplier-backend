...
exports.regenerateSupplierPassword = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return res.status(404).json({ success: false, message: 'No user account found for this supplier.' });
    }
    ...
    // Emit real-time force logout event to user
    try {
      const { getIo } = require('../services/socket.service');
      const io = getIo();
      io.to(`user_${user.id}`).emit('force_logout', {
        message: 'Your password has been regenerated by an administrator. Please log in again with your new password.'
      });
      console.log(`Socket force_logout event emitted to user_${user.id}`);
    } catch (socketError) {
      console.warn('Failed to emit force_logout socket event:', socketError.message);
    }
    ...
    if (transporter) {
      try {
        await transporter.sendMail({
          from: `"B2B Supplier Portal" <${process.env.SMTP_USER}>`,
          to: user.email,
          subject: 'Your Supplier Account Password Reset',
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 10px;">
              <h2 style="color: #1e3a8a; text-align: center;">B2B Supplier Portal</h2>
              <p style="color: #334155; font-size: 16px;">Hello ${supplier.name},</p>
              <p style="color: #334155; font-size: 16px;">Your supplier account password has been regenerated by the administrator. Here are your new login credentials:</p>
              <div style="background-color: #f1f5f9; padding: 15px; border-radius: 8px; margin: 20px 0;">
      ...
      console.log(`\n=========================================`);
      console.log(`SMTP NOT CONFIGURED - New credentials for ${user.email}: ${plainPassword}`);
      console.log(`=========================================\n`);
    }
    ...
    if (emailSent) {
      return res.status(200).json({
        success: true,
        message: 'Password regenerated and sent to supplier email successfully.',
      });
    } else {
      return res.status(200).json({
        success: true,
        message: 'Password regenerated, but email could not be sent. New password is: ' + plainPassword,
